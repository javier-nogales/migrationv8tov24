...
/////////////////////////////////////////////////////////////////////////////////////////
	public VaadinUserMenu(final UII18NService i18n,
		...
		Collection<R01UILangButton> langButtons = this.createLanguageChangeButtons(supportedLangs);
		_langChangeButtons = new VaadinLangButtons(langButtons);
		if (_langChangeButtons.hasData()) {
			StreamSupport.stream(_langChangeButtons.spliterator(),
								 false)	// not parallel
		...
		_btnSignOut.addClickListener(event -> {
										// the redir to the login or logout page MUST done BEFORE closing the session,
										...
										// --- Redir
										// if the securityContext contains a logout url redir to that url
										if (securityContext.getLogoutUrl() != null) {
											// the logout flow is like:
											//		[UI]         [SecurityRestServices]
											//       |                       |
											//		 +----redir----> /api/{providerId}/users/logout
											//       |                       |
											//       |                      [ ] ----> register logout
											//       |                       |
											//   /logout <------redir--------+
											...
											// ui logout (reeuse the login url replacing "login" by "logout")
											String uiLogoutUrl = securityContext.getLoginUrl().asString()
																							  .replace("login","logout");
											// security REEST services logout
											Url logoutUrl = securityContext.getLogoutUrl()	// usually this is like /{securityRestServicesWebApp}/api/{securityProviderId}/logout
																		   .joinWith(UrlQueryStringParam.of("uiLogoutUrl",uiLogoutUrl));	// see RESTResourceForSecurityServicesBase
											Page.getCurrent()
												.setLocation(logoutUrl.asString());
										}
										// otherwise if the securityContext contains a login url redir to that
										else if (securityContext.getLoginUrl() != null) {
											Page.getCurrent()
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	public void setUserInfoButtonCaption(final String buttonCaption) {
		_btnUser.setCaption(buttonCaption);
	}
	public void setUserInfoPopupOpen(final boolean open) {
		if (open) {
			_popup.setPopupVisible(true);
		} else {
...
//	LANGUAGE BUTTONS
/////////////////////////////////////////////////////////////////////////////////////////
	public Collection<R01UILangButton> createLanguageChangeButtons(final Language... supportedLangs) {
		if (CollectionUtils.isNullOrEmpty(supportedLangs)) return null;

		return Stream.of(supportedLangs)
					 .map(lang -> {
	...
	@RequiredArgsConstructor
	public class VaadinLangButtons
	  implements Iterable<R01UILangButton>,
		...
		public void setStyle(final String style) {
			if (CollectionUtils.isNullOrEmpty(_langButtons)) return;
			for (R01UILangButton langBtn : _langButtons) {
				langBtn.setStyle(style);
			}
		}
	}
	@Accessors(prefix="_")
	public class R01UILangButton
...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	public void updateI18NMessages(final UII18NService i18n) {
		...
		if (_component != null
		 && _component instanceof VaadinViewI18NMessagesCanBeUpdated i18nAwareComp) {
		...
		_langChangeButtons.setCurrent(i18n.getCurrentLanguage());
	}
}
